...
public class AuthProvider {
    ...
    public String updateSonSortTwo(Map map) {
        StringBuffer buffer = new StringBuffer("update sys_auth set ");
        buffer.append("sort = CONCAT(#{sort},if(length(sort)&1,concat('0',sort),sort)) where length(sort)<9");
        return buffer.toString();
    }
    ...
    public String updateSortList(Map map) {
        ...
        if (type.equals("insert")) {
            if (insertType != null) {
                if (insertType == 1) {
                    if (pid == null)
                        buffer.append("select sort from sys_auth where sort>=#{sort} order by sort desc");
                    else
