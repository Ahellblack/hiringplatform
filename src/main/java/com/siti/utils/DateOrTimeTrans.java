...
public class DateOrTimeTrans {
     ...
     *
     * @param date 指定日期
     * @param n    前n/后n,正数往后，负数往前
     * @return 月份，格式yyyy-MM
     */
    public static String getNextMonthOfDateByNextNum(String date, int n) throws ParseException {
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
        Calendar dateCalendar = Calendar.getInstance();
        dateCalendar.setTime(sdf.parse(date));

        dateCalendar.add(Calendar.MONTH, n);
        return DateTransform.Date2String(dateCalendar.getTime(),"yyyy-MM");
    }
